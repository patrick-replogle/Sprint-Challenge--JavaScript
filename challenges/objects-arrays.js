...
Once you have the new array created, sort the universities alphabetically and log the result. */
 function universities() {
  ...
  for(let i = 0; i < graduates.length; i++) {
    result.push(graduates[i].university)
  }
  return result.sort();
};
...
const contactInfo = graduates.map(element => `${element.first_name} ${element.email}`);
...
function uni() {
  let result = [];
  for(let i = 0; i < graduates.length; i++) {
    let contains = graduates[i].university;
    if(contains.includes('Uni')){
...
*/
const animalNames = [];
const newArray = zooAnimals.forEach(element => animalNames.push(`Name: ${element.animal_name}, Scientific: ${element.scientific_name}`));
...
*/
const populationTotal = zooAnimals.reduce((total, currentValue) => total += currentValue.population, 0);
